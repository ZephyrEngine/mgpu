...
Application::Application() {
  ...
  const MGPUTextureCreateInfo checkerboard_texture_create_info{
  ...
  MGPU_CHECK(mgpuDeviceCreateTexture(m_mgpu_device, &checkerboard_texture_create_info, &m_mgpu_checkerboard_texture));

  const MGPUTextureViewCreateInfo checkerboard_texture_view_create_info{
  ...
  MGPU_CHECK(mgpuTextureCreateView(m_mgpu_checkerboard_texture, &checkerboard_texture_view_create_info, &m_mgpu_checkerboard_texture_view));

  u32 checkerboard_texture_data[]{
    0xFFFFFFFFu, 0xFF000000u,
    0xFF000000u, 0xFFFFFFFFu
  };
  const MGPUTextureUploadRegion texture_upload_region{
    .offset = {.x = 0u, .y = 0u, .z = 0u},
    .extent = {.width = 2u, .height = 2u, .depth = 1u},
    .mip_level = 0u,
    .base_array_layer = 0u,
    .array_layer_count = 1u
  };
  MGPU_CHECK(mgpuQueueTextureUpload(mgpu_queue, m_mgpu_checkerboard_texture, &texture_upload_region, checkerboard_texture_data));

  const MGPUSamplerCreateInfo nearest_sampler_create_info{
  ...
  MGPU_CHECK(mgpuDeviceCreateSampler(m_mgpu_device, &nearest_sampler_create_info, &m_mgpu_nearest_sampler));
  ...
  std::vector<MGPUResourceSetBinding> resource_set_bindings{
    ...
    },
    {
      ...
      .type = MGPU_RESOURCE_BINDING_TYPE_TEXTURE_AND_SAMPLER,
      .texture = {
        .texture_view = m_mgpu_checkerboard_texture_view,
...
Application::~Application() {
  ...
  mgpuTextureViewDestroy(m_mgpu_checkerboard_texture_view);
  mgpuTextureDestroy(m_mgpu_checkerboard_texture);
